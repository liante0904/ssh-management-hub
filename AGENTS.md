...
### 11. IDE 스타일 Split Panel 패턴
- `Database.jsx`와 `Logs.jsx`는 왼쪽 탐색기(Table List, File List)와 오른쪽 뷰어 영역이 분리된 IDE 스타일 레이아웃을 사용합니다.
- `leftOpen` 상태값을 통해 왼쪽 패널을 토글할 수 있으며, `transition` 속성으로 부드러운 애니메이션이 적용되어 있습니다.

### 12. DB 코멘트 및 직접 쿼리
- `Database.jsx`에서 테이블/컬럼 코멘트를 수정하면 `api.dbComment`를 통해 DB에 즉시 반영됩니다.
- 쿼리 모드는 `api.dbSqlQuery`를 사용하며, 보안상 **SELECT** 쿼리 위주로 사용하도록 유도합니다.

### 13. PDF 아카이브 대량 처리 주의사항
- `PdfArchive.jsx`의 `bulkReprocess` 및 `quickReprocessRecent100` 기능은 백엔드에 상당한 부하를 줄 수 있습니다.
- 재처리 요청 전 반드시 `confirm()`을 통해 사용자 승인을 받도록 구현되어 있으며, 에이전트 역시 자동화된 대량 재처리는 지양해야 합니다.
